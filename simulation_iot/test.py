...
def write_read(x):
    tmp=str(x)
    arduino.write(bytes(tmp, 'utf-8'))
    # data = arduino.readline()
    # return data
pinsard={0:4,1:6,2:8,3:10}
# ypin={0:3,1:6,2:9,3:12}
...
vcount=[5,2,3,4]  #Total Vehicles incoming
dcount=vcount.copy() #Vehicles yet to come
flag=False
in_lane=vcount.copy()
incoming=[0,0,0,0]
...
# Generating vehicles in the simulation
def generateVehicles():
    global vcount,in_lane,incoming
    i=0
    while (True):
        vehicle_type = random.choice(allowedVehicleTypesList)
        lane_number = random.randint(1, 2)
        ...
        if direction_number==0:
            if vcount[0]>0:
                Vehicle(lane_number, vehicleTypes[vehicle_type], direction_number, directionNumbers[direction_number], will_turn)
                vcount[0]=vcount[0]-1
                incoming[direction_number]+=1
                in_lane[direction_number]=incoming[direction_number]-vehicles[directionNumbers[direction_number]]['crossed']
            else:
                pass
            ...
            if vcount[1]>0:
                ...
                vcount[1]=vcount[1]-1
                incoming[direction_number] += 1
                in_lane[direction_number] = incoming[direction_number] - vehicles[directionNumbers[direction_number]][
                    'crossed']
            else:
                pass
...
def nvehcount():
    global vcount,flag,dcount
    if flag==False:
        flag=True
        time.sleep(15)
    while (True):
        vcount[0] = random.randint(2,5)
        vcount[1] = random.randint(2, 5)
        vcount[2] = random.randint(2, 5)
        vcount[3] = random.randint(2, 5)
        dcount=vcount.copy()
        time.sleep(15)               ##############################################################   CONTROL DENSITY OF TRAFFIC ##################################################
...
class Main:
    ...
    fontin=pygame.font.Font(None,40)
    ...
    thread4 = threading.Thread(name="nvehcount", target=nvehcount, args=())  # recount vehicles
    thread4.daemon = True
    thread4.start()

    # thread5 = threading.Thread(name="write_read", target=write_read, args=())  # recount vehicles
    # thread5.daemon = True
    # thread5.start()
    ...
    while True:
        ...
        # display Vehicle count
        k = fontin.render("left:"+str(vcount[0])+"/"+str(dcount[0])+"||"+str(incoming[0])+"||"+str(in_lane[0]), True, white, black)
        screen.blit(k, (20,280))
        k = fontin.render(" top:"+str(vcount[1])+"/"+str(dcount[1])+"||"+str(incoming[1])+"||"+str(in_lane[1]), True, white, black)
        screen.blit(k, (800,20))
        k = fontin.render("right:"+str(vcount[2])+"/"+str(dcount[2]), True, white, black)
        screen.blit(k, (1250,550))
        k = fontin.render("down:"+str(vcount[3])+"/"+str(dcount[3]), True, white, black)
        screen.blit(k, (450,700))
